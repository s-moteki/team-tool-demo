  ...
  <transition name="fade-fast" mode="out-in">
    <div class="attendance has-text-centered" v-if="attendance">
      ...
      <CardModal v-if="showForm" @close="showForm = null">
        ...
        <i v-if="showForm.attendance_id != null" class="fas fa-trash-alt has-text-danger delete-icon" slot="footer" @click="showAlert = true"></i>
      </CardModal>

      <!-- 削除モーダル -->
      <CardModal v-if="showAlert || showAlertDelete" @close="showAlertMail = null, showAlertDelete = null">
        <div class="input-title" slot="body">
          <p v-if="showAlertMail">提出しますか?</p>
          <p v-else>削除しますか？</p>
        </div>
        <div slot="footer" class="buttons is-centered">
          <button v-if="showAlertMail" class="button is-success" @click="sendMail">はい</button>
          <button v-else class="button is-success" @click="deleteAttendance">はい</button>
      ...
      <!-- 提出ボタン -->
      <button  :disabled="submission" class="button send" @click="showAlertMail = true">
