      ...
      <div v-if="result" class="container">
        <div class="columns has-text-centered">
          <div class="column is-half">
            ...
            <p class="subtitle is-2">{{(result.you.minute/60).toFixed(1)}}時間</p>
            <p class="is-size-4">今月の残業日数</p>
            <p class="subtitle is-2">{{result.you.workdate}}日</p>
            <p class="is-size-4">1日の平均残業時間</p>
            <p class="subtitle is-2">{{((result.you.minute/60)/30).toFixed(2)}}時間</p>
          </div>
          <div class="column is-half">
            <p class="subtitle is-4"><i class="fas fa-2x fa-users"></i> 全体</p>
            <p class="is-size-4">今月の平均残業時間</p>
            <p class="subtitle is-2">{{(result.group.minute/60).toFixed(1)}}時間</p>
            <p class="is-size-4">今月の平均残業日数</p>
            <p class="subtitle is-2">{{result.group.workdate.toFixed()}}日</p>
            <p class="is-size-4">1日の平均残業時間</p>
            <p class="subtitle is-2">{{((result.group.minute/60)/30).toFixed(2)}}時間</p>
          </div>
        </div>
      </div>
