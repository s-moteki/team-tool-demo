...
export default {
  ...
  methods: {
    ...
    // 勤怠削除
    deleteAttendance () {
      axios.delete(`https://moteki1236.sakura.ne.jp/team-tool/api/attendances/${this.showForm.attendance_id}`)
      .then(response => {
        alert('削除が完了しました')
        this.showForm = null
        this.changeYM()
    ...
    // 勤怠情報更新
    editAttendance () {
      axios.put(`https://moteki1236.sakura.ne.jp/team-tool/api/attendances/${this.showForm.attendance_id}`,this.showForm)
      .then(response => {
        alert('更新が完了しました')
        this.showForm = null
        this.changeYM()
    ...
    // 勤怠追加
    addAttendance () {
      axios.post(`https://moteki1236.sakura.ne.jp/team-tool/api/attendances`,this.showForm)
      .then(response => {
        alert('投稿が完了しました')
        this.showForm = null
        this.changeYM()
    ...
    // 選択中月日の勤怠情報の取得
    updateSubmissions () {
      axios.put(`https://moteki1236.sakura.ne.jp/team-tool/api/updatesubmissions`, this.attendance)
      .then(response => {
        alert('勤怠報告を提出しました。')
        this.changeYM()
      }).catch(err => {
    ...
    changeYM () {
      ...
      axios.get(`https://moteki1236.sakura.ne.jp/team-tool/api/attendances?id=${this.user_oid}&year=${this.selectedYM.slice(0, 4)}&month=${this.selectedYM.slice(-2)}`)
      .then(response => {
  ...
  mounted () {
    axios.get(`https://moteki1236.sakura.ne.jp/team-tool/api/attendances?id=${this.user_oid}&year=${this.thisMonth().slice(0, 4)}&month=${this.thisMonth().slice(-2)}`)
    .then(response => {
      this.attendance = response.data.attendance
