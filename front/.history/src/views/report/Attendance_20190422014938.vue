  ...
  methods: {
    ...
    isSubmission () {
      if (this.selectedYM == this.thisMonth()) {
        return this.attendance.length == 0 ? false : parseBoolean(this.attendance[0].submission)
      } else {
        return this.attendance.length == 0 ? true : parseBoolean(this.attendance[0].submission)
      }
    }
  },
  mounted () {
    ...
    this.selectedYM = '201901'
    this.submission = this.isSubmission()
    alert(this.isSubmission())
  }
}
</script>
