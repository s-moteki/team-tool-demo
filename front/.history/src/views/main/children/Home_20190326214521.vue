    ...
    <transition name="fade-fast" mode="out-in">
      <div class="container" v-if="filterEvents">
        <div v-for="i in 3" :key="i" class="event-section">
          <p class="is-size-5">{{months[i-1]}}月</p>
          <div v-if="filterEvents.get(months[i-1]) == null">
            <p class="is-size-5 no-event">{{months[i-1]}}月の予定はございません</p>
          </div>
          <div v-else class="events">
            <button class="button is-outlined"
              v-for="(event, index ) in filterEvents.get(months[i-1])"
              :key="index"
...
export default {
  ...
  methods: {
    ...
    addAndGetMonth (num) {
      return parseInt(new Date().getMonth()) + num
    },
