  ...
  <section class="hero is-fullheight">
    ...
    <div class="hero-body">
      <div class="container">
        ...
        <h2 class="subtitle" v-for="event in accountEvensts" :key="event.id">
          {{event.subject}}
        </h2>
      </div>
    </div>
...
import * as KJUR from "jsrsasign/lib/jsrsasign"
...
export default {
  ...
  computed: {//算出プロパティ
    ...
    accountMail: function() {
      return sessionStorage.userSigninName
    },
    accountMailList: function() {
      return sessionStorage.TestMail
    },
    accountEvensts: function() {
      return sessionStorage.TestEvent?JSON.parse(sessionStorage.TestEvent):null
    }
  },
  data(){
    return{
...
var isAuthenticated = (sessionStorage.accessToken != null && sessionStorage.accessToken.length > 0);
  ...
  if (isAuthenticated) {
    ...
    getUserInboxMessages(function(messages, error){
      sessionStorage.TestMail = JSON.stringify(messages)
      if(error){
        sessionStorage.TestMail = error
      }
    });

    getUserEvents(function(messages, error){
      sessionStorage.TestEvent = JSON.stringify(messages)
      if(error){
        sessionStorage.TestEvent = error
      }
    });
