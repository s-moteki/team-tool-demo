<template>
  <section class="section" v-if="boards">
    <div class="container">
      <div class="wrap"><h1>お知らせ</h1>{{$_mixinUtil_splitArray(boards).length}}</div>
      <div class="card-area">
        <div class="row columns" v-for="(rowBoards, index) in $_mixinUtil_splitArray(boards)" :key = index>
      ...
      <CardModal v-if="targetModal" v-bind = "targetModal" @close="targetModal = false">
        ...
        <p class="subtitle has-text-centered created-info" slot="footer">
          {{`${targetModal.user}（${parseDate(targetModal.created_at)}）`}}
        </p>
      </CardModal>
