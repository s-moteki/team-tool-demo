    ...
    <transition name="fade-fast" mode="out-in">
      <div class="container" v-if="notices">
        ...
        <CardModal v-if="showForm" @close="closeForm()">
          ...
          <button id="addButton" class="button is-fullwidth" slot="footer" @click="deleteNotice()">投稿</button>
        </CardModal>

        <!-- 削除モーダル -->
        <CardModal v-if="deleteSign" @close="deleteNotice(null)">
          <div class="input-title" slot="body">
            <p>投稿を削除しますか?</p>
          </div>
          <div slot="footer" class="buttons has-addons">
            <button class="button is-success is-left">はい</button>
            <button class="button is-danger">いいえ</button>
          </div>
        </CardModal>
