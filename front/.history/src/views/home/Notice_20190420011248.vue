  ...
  <section class="section">
    <transition name="fade-fast" mode="out-in">
      <div class="container" v-if="notices">
        ...
        <!-- 更新フォーム -->
        <CardModal v-if="updateNotice" @close="setUpdateSign(null)">
          <div class="input-title" slot="title">
            <p class="title has-text-centered">更新内容の入力</p>
            <input v-model="updateNotice.title" class="input" type="text" placeholder="タイトル">
          </div>
          <input v-model="updateNotice.subtitle" class="input" type="text" placeholder="サブタイトル" slot="subtitle">
          <textarea v-model="updateNotice.text" class="textarea has-fixed-size" placeholder="本文" slot="body"></textarea>
...
export default {
  ...
  methods: {
    ...
    // お知らせ情報更新
    updateNotice (id) {
      axios.put(`http://localhost:8888/api/notices/${id}`, this.updateNotice)
        .then(response => {
          alert('更新が完了しました')
          this.showLoading = true
          this.notices = response.data
          this.showLoading = false
          this.setUpdateSign(null)
        })
        .catch((error) => {
          alert('更新に失敗しました')
          console.log(error)
