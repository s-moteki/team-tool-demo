  ...
  async sendMail (data, date) {
    ...
    if (token) {
      ...
      reader.onload = async ()=>{
        // 送信メール情報
        const mail = {
          subject: `${date.slice(0, 4)}年${date.slice(-2)}月 勤怠報告 ${sessionStorage.userDisplayName}`,
          toRecipients: [
            {
              emailAddress: {
          ...
          body: {
            content: `<h1>${date.slice(0, 4)}年${date.slice(-2)}月 勤怠報告</h1><h2>${sessionStorage.userDisplayName}</h2>本メールはTeam-Toolによる自動送信です。`,
            contentType: "html"
          },
          Attachments: [
            {
